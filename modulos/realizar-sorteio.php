                            ...
                            <div class="form-row">
                                <div class="col-md-5 offset-md-4">
                                    ...
                                    <select data-placeholder="Escolha a Partida..." id="filtro" name="filtro" class="form-control obrigatorios chosen-select" required>
                                        ...
                                        <?php
                                        $query = $conn->prepare("
                                        SELECT *, COUNT(id_confirmacao) AS nro_confirmados FROM partidas p 
                                        INNER JOIN locais l ON (l.id_local = p.local_partida)
                                        INNER JOIN confirmacoes c ON (c.partida_confirmacao = p.id_partida)
                                        WHERE status_partida = 1 HAVING COUNT(id_confirmacao) >= (p.jogadoresTime_partida * 2)
                                        ");
