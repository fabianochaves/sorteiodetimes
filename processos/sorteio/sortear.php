...
//CONSULTA SORTEIO
$busca_sorteio = $conn->prepare("SELECT * FROM sorteios WHERE partida_sorteio = '$id_partida'");
try {
    $busca_sorteio->execute();
} catch (PDOException $e) {
    $e->getMessage();
}

if ($busca_sorteio->rowCount() > 0) {
    echo "existe";
    return false;
} else {
...
?>
    ...
    <div id="lista_times">
        ...
        ?>
            <center>
                <p><b>
                        <font color='blue'>Time <?php echo $t; ?></font>
                    </b></p>
            </center>
            <table class="table table-bordered" style="width: 100%">
                ...
                ?>
                    <tr>
                        <td>
                            <center><?php echo $nome_jogador . " " . $goleiro; ?></center>
                        </td>
                        <td>
                            <center><?php echo $nivel_jogador; ?></center>
                        </td>
                    </tr>
                <?php
                }
...
?>
 ...
 <?php
        //UPDATE SORTEIO REALIZADO
        $update_partida = $conn->prepare("
    UPDATE partidas SET sorteio_partida = '$id_sorteio', status_partida = 3 WHERE id_partida = '$id_partida'
    ");
        try {
            $update_partida->execute();
        } catch (PDOException $e) {
            $e->getMessage();
        }
    }
